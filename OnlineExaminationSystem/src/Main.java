...
public class Main {
    ...
    // Step 3: Fetch the selected exam
    Exam selectedExam = exams.get(choice - 1);

    // Step 4: Retrieve random questions for the exam
    ArrayList<Question> questionsToAsk = selectedExam.getQuestions(); // Ensure this method exists and works as expected
    Collections.shuffle(questionsToAsk);  // Randomize the order of questions
    ...
    // Step 6: Display the student's score
    double grade = (double) score / questionsToAsk.size() * 100; // Calculate percentage
    System.out.println("Exam completed. You scored: " + score + "/" + questionsToAsk.size() + " (" + grade + "%)");

    // Step 7: Save the result (if needed)
    if (loggedInUser instanceof Student) {
        Result result = new Result((Student) loggedInUser, selectedExam, score); // Pass score to the Result constructor
        ((Student) loggedInUser).addResult(result); // Ensure `addResult` is implemented in the Student class
        storeResult(((Student) loggedInUser).getUsername(), selectedExam, score); // Store result in file
        System.out.println("Your result has been saved.");
    } else {
        System.out.println("Error: Only students can take exams. Result not saved.");
    }
}
    ...
    private static void storeResult(String studentName, Exam exam, int score) {
    String result = "Student: " + studentName + ", Exam: " + exam.getTitle() + 
                    ", Score: " + score + ", Date: " + new java.util.Date();

    try (BufferedWriter bw = new BufferedWriter(new FileWriter("exam_results.txt", true))) {
        bw.write(result);
        bw.newLine();  // Add a new line after each result
    } catch (IOException e) {
        System.out.println("Error saving the result: " + e.getMessage());
    }
}
